...
#
# Adjusted for KV450 blue sub-sample (TB>3) by Shun-Sheng Li
#
# ATTENTION:
...
class kv450_blue_likelihood(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        try:
            fname = os.path.join(self.data_directory, 'DATA_VECTOR/KV450b_xi_pm_tomographic.dat')
            parser_mp.existing_file(fname)
        except:
            raise io_mp.ConfigurationError('KiDS+VIKING-450 blue data not found.')
        ...
        if self.integrate_Bessel_with in ['brute_force', 'cut_off']:
            ...
            #
            # We first apply this to a running value ll, in order to count the total number of ll's, called nl.
