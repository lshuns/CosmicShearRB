...
#
# Adjusted to produce theoretical xi_pm vector by Shun-Sheng Li
#
# ATTENTION:
...
class kv450_theory_vector(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        folder_name = os.path.join(self.data_directory, 'RESULTS')
        ...
        self.__write_out_vector_in_list_format(self.xi_obs, fname_prefix=self.prefix_signal)
        ...
        if (self.shift_n_z_by_D_z):
            ...
            for zbin in xrange(self.nzbins):
                ...
                z_mod = self.z_p + self.D_z[param_name]
        ...
        if (self.include_dm):
            for zbin in xrange(self.nzbins):
                dm_per_zbin[:, zbin] = np.ones(self.ntheta) * self.dm[zbin]
        ...
        if (self.include_dc1):
            for zbin in xrange(self.nzbins):
                dc1_per_zbin[:, zbin] = np.ones(self.ntheta) * self.dc1[zbin]
        if (self.include_dc2):
            for zbin in xrange(self.nzbins):
                dc2_per_zbin[:, zbin] = np.ones(self.ntheta) * self.dc2[zbin]
        ...
        self.__write_out_vector_in_list_format(self.xi, fname_prefix=self.prefix_theory)
    ...
    def __write_out_vector_in_list_format(self, vec, fname_prefix='your_filename_prefix_here'):
        ...
        fname = os.path.join(self.data_directory , 'RESULTS/{:}_cut_to_{:}.dat'.format(fname_prefix, self.mask_suffix))
    ...
    def one_dim_index(self, Bin1, Bin2):
        if Bin1 <= Bin2:
            ...
            return int(Bin2 + self.nzbins * Bin1 - (Bin1 * (Bin1 + 1)) / 2) # modified by shun-sheng due to data type problem
        else:
            # return Bin1 + self.nzbins * Bin2 - (Bin2 * (Bin2 + 1)) / 2
            return int(Bin1 + self.nzbins * Bin2 - (Bin2 * (Bin2 + 1)) / 2) # modified by shun-sheng due to data type problem
